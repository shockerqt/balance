...
import { formatMacroGrams } from '@/lib/nutrition';
...
export const MacroGrid: React.FC<{ totals: MacroTotals; targets: DayTargets }> = ({
  ...
  return (
    <View style={[styles.grid, { gap: theme.space.sm }]}>
      {macros.map((macro) => {
        const value = formatMacroGrams(macro.value);
        const target = formatMacroGrams(macro.target);
        return (
          <Card
            key={macro.label}
            raised
            ...
            radius="md"
            style={styles.cell}
            accessibilityLabel={`${macro.label}: ${value} de ${target} gramos`}>
